#!/usr/bin/env python3
"""
nautilus_catalog.py — Nautilus catalog end-to-end validation.
...
class NautilusCatalogValidator:
    ...
    def __init__(self, target_date: Optional[str] = None):
        ...
        self._catalog_inspect_report: Optional[dict] = None
    ...
    def _get_catalog_inspect_report(self) -> dict:
        if self._catalog_inspect_report is None:
            from inspect_catalog import inspect_catalog_depth

            self._catalog_inspect_report = inspect_catalog_depth(
                self._catalog,
                fail_limit=5,
                date_str=self._date_str,
            )
        return self._catalog_inspect_report
    ...
    def _ck_report_valid(self) -> dict:
        ...
        required = [
            ...
            "crossed_book_events_total", "gap_rate", "per_symbol_gaps",
            "per_symbol_crossed_books", "symbols_processed",
            "venues", "ts_ranges",
        ]
    ...
    def _ck_gap_fields_valid(self) -> dict:
        ...
        for vname, vdata in rpt.get("venues", {}).items():
            ...
            if "crossed_book_events" not in vdata:
                venue_missing.append(f"{vname}.crossed_book_events")
    ...
    def _ck_crossed_book_venue(self, venue_name: str) -> dict:
        """Catalog spread sanity must be evaluated independently per venue."""
        report = self._get_catalog_inspect_report()
        venue = report.get("venues", {}).get(venue_name, {})
        checked_instruments = venue.get("checked_instruments", 0)
        checked_snapshots = venue.get("checked_snapshots", 0)
        failed_instruments = venue.get("failed_instruments", [])

        # If a venue was not selected for this date, don't fail the validator.
        if checked_instruments == 0 and checked_snapshots == 0:
            return {
                "name": f"crossed_book_{venue_name}",
                "passed": True,
                "details": {
                    "reason": "no instruments for venue",
